namespace SMapEditor.Editor {
	...
	using SMapEditor.Core;

	/// <summary>
	/// 需要类：JujubeScene，JujubeCombinerWindow
	/// </summary>
	[CustomEditor(typeof(SMapEditor.Core.JujubeRenderer), true)]
	public class JujubeRenderer_Inspector : MoenenInspector {
		...
		private void GUI_ControlButtons () {
			...
			var jTarget = target as SMapEditor.Core.JujubeRenderer;
			// Reload On Change
			if (
				jTarget.Map != PrevMap ||
			...
			if (jTarget.Map != null && jTarget.Palette != null) {
				...
				LayoutH(() => {
					...
					if (PrefabStageUtility.GetCurrentPrefabStage() == null && !PrefabUtility.IsPartOfPrefabAsset(jTarget)) {
				...
				LayoutH(() => {
					...
					if (GUI.Button(GUIRect(0, ITEM_HEIGHT), new GUIContent(" Export Combined Mesh", GetJujubeImage("Mesh Icon.png")))) {
						//TODO: SMapEditor -> COMBINE Window 暂时没有
